...
.subject-card {
  ...
  perspective: 1000px;
  cursor: pointer;
  margin-bottom: 70px; /* Add vertical space between rows */
  margin-right: 40px;   /* Add horizontal space if cards are side by side */
}
.subject-card .face {
  position: absolute;
...
.subject-card.flipped .front { transform: rotateY(180deg); }
.subject-card.flipped .back  { transform: rotateY(360deg); }
.subject-title { font-weight: 600; }
.subject-desc  { font-size: 0.95rem; line-height: 1.4; }
...
$programsByLevel[$level][$program][] = [
  ...
  <u class="typing-text">Programs Offered at Massachusetts Institute of Technology</u>
</h1>
...
.typing-text {
  ...
  animation: typing 12s steps(53, end) infinite, blink 0.7s step-end infinite;
  white-space: nowrap;
}

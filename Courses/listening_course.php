...
<head>
  ...
  <style>
   body {
  margin: 0;
  background: linear-gradient(135deg,rgb(19, 31, 82), #1e3c72);
...
.video-card::before {
  ...
  width: 200%;
  height: 200%;
  background: linear-gradient(120deg, rgba(0,198,255,0.3), rgba(0,114,255,0));
  opacity: 0;
...
<h1>🎧 Listening Course Playlist</h1>
...
<div class='video-card'>
    ...
    <div class='label'>Listening Video $num</div>
</div>";
    }
} else {
    echo "<p style='color: red;'>No videos found for Listening section.</p>";
}
$conn->close();
?>
</div>
...
<script>
  // Fix video card size when exiting fullscreen
  ...
  // Trigger 3D entrance effect on load
